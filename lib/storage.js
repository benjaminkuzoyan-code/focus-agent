...
(() => {
  ...
  async function set(key, value) {
    try {
      ...
      // chrome.storage.local is 10 MB; when it's full every save fails. Say so
      // somewhere the panel can show, instead of silently losing the thread.
      FA.storeError = /quota/i.test(e.message) ? "Storage is full — tick ✓ on old assignments to free space." : `Couldn't save: ${e.message}`;
      throw e;
    }
  }

  // Per-assignment memory (chat thread, attached file text, practice tests,
  // screenshot thumbnails) is the only thing that grows without bound. Once
  // an assignment has been done for a month, keep the small facts and drop
  // the heavy ones.
  const PRUNE_AFTER_MS = 30 * 24 * 60 * 60 * 1000;
  const HEAVY = ["thread", "files", "tests", "chatHistory"];
  function prune(meta) {
    const cutoff = Date.now() - PRUNE_AFTER_MS;
    for (const m of Object.values(meta)) {
      if (m && m.done && m.doneAt && m.doneAt < cutoff) for (const k of HEAVY) delete m[k];
    }
    return meta;
  }
  ...
  FA.store = {
    ...
    async patchAssignmentMeta(assignmentId, patch) {
      ...
      await set("assignmentMeta", prune(meta));
    },
    /**
    ...
    async startSession(assignment, plannedMin, extra = {}) {
      const session = {
        ...
        checkins: 0,
        lastNudgeAt: 0,
        lastActiveAt: Date.now(), // last sign of the student (worker: chrome.idle, tabs, doc edits; panel: taps)
        idleAskAt: 0,             // when the "still there?" went out; 0 = not asked
        ...extra,
      };
    ...
    async endSession(endedBy = "user", extra = {}) {
      const s = await this.getActiveSession();
      if (!s) return null;
      // An idle stop ends the session at the last sign of life, not at the
      // moment the watchdog noticed — a forgotten clock never logs the gap.
      const endedAt = extra.endedAt || Date.now();
      const actualMin = Math.max(1, Math.round((endedAt - s.startedAt) / 60000));
      const record = {
        endedBy,
        ...extra,
        id: "s" + s.startedAt,
        ...
        endedAt,
        idleMin: extra.idleMin || 0, // quiet minutes NOT counted in actualMin
