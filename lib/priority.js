...
(() => {
  ...
  FA.isBehind = function isBehind(a) {
    ...
    if (a.finished || FA.isStale(a)) return false;
    return Boolean(a.overdue) || (a.dueDate ? FA.hoursUntil(a.dueDate) < 0 : false);
  };

  /**
   * The overdue grace window. Blackbaud calls anything past due that the
   * student never ticked "complete" OVERDUE — forever. Most of those are
   * done (readings, in-class work, homework handed in on paper); the teacher
   * never asks for the tick. So past-due work counts as "behind" only for
   * OVERDUE_GRACE_DAYS after the due date; after that it's "stale": out of
   * the ranking, folded away with the finished work, and it comes back on
   * top only if the teacher marks it MISSING (the portal's real signal).
   */
  FA.OVERDUE_GRACE_DAYS = 5;
  FA.isStale = function isStale(a) {
    if (!a || a.missing || a.finished || !a.dueDate) return false;
    return FA.hoursUntil(a.dueDate) < -24 * FA.OVERDUE_GRACE_DAYS;
  };
  ...
  FA.rankAssignments = function rankAssignments(assignments, meta, sessions) {
    return assignments
      .filter((a) => !meta?.[a.id]?.done && !a.finished && !FA.isStale(a)) // finished = completed/graded in the portal; stale = past the overdue grace window
      .map((a) => ({
        ...a,
