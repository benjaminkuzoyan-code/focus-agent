 ...
 *
 *   FA.pdfText.fromUrl(url, opts)     → { text, pages, truncated }  (fetched by the worker: Drive-aware)
...
(() => {
  ...
  async function extract(getDocumentArg, { maxPages = 80, maxChars = 200000 } = {}) {
    ...
    return { text, pages: doc.numPages, truncated: doc.numPages > n || total > maxChars };
  }

  FA.pdfText = {
    async fromUrl(url, opts) {
      const bytes = await FA.fetchPdfBytes(url);
      return extract({ data: bytes }, opts);
    },
