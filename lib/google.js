...
const WEB_CLIENT_ID = "248896287493-4640r61qu86tcfqabid7v8mcbdk56kjm.apps.googleusercontent.com";
  ...
  async function status() {
    ...
    if (state?.version === 1) return {
      ...
      userDisconnected: state.userDisconnected === true,
    };
  ...
  async function acquireWebToken(interactive, generation) {
    ...
    assertCurrent(generation);
    if (!interactive && saved?.access_token && saved.expires_at > Date.now()) return { token: saved.access_token, door: "web" };
    // A cookie session or login hint alone cannot prove the student's selection.
    // Without a verified prior binding, only an explicit chooser may replace it.
    if (!interactive && (!saved?.email || saved.emailVerified !== true)) throw authError("authorization", "web");
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (info.email_verified === true && typeof info.email === "string" && info.email.trim()) providerEmail = info.email;
      }
    } catch {}
    assertCurrent(generation);
    if (!interactive && (!providerEmail || saved.email.toLowerCase() !== providerEmail.toLowerCase())) throw authError("authorization", "web");
    email = providerEmail || email;
    await chrome.storage.local.set({ [WEB_TOKEN_KEY]: { access_token, expires_at, email, emailVerified: Boolean(providerEmail), writer } });
    assertCurrent(generation);
  ...
  async function getToken(interactive, generation = connectionGeneration) {
    ...
    assertCurrent(generation);
    if (!interactive && state.userDisconnected) throw authError("authorization", null);
  ...
  async function forgetToken({ token, door }, generation = connectionGeneration) {
    ...
    if (door === "web") {
      ...
      await chrome.storage.local.set({ [WEB_TOKEN_KEY]: { email: saved.email || "", emailVerified: saved.emailVerified === true, expires_at: 0, writer } });
    }
  ...
  async function markAuthorization(error, generation, operation) {
    assertCurrent(generation);
    if (error.category !== "authorization" || error.attempts?.some(a => a.category !== "authorization")) return;
    const saved = await chrome.storage.local.get(AUTH_STATE_KEY);
    const state = await status();
    assertCurrent(generation);
    if (operation < latestSuccess) return;
    if (!state.userDisconnected && state.everConnected && state.status !== "reconnect") await chrome.storage.local.set({ [AUTH_STATE_KEY]: { ...state, status: "reconnect", reason: "authorization", writer, session: saved[AUTH_STATE_KEY]?.session || null } });
  }
  async function api(url, { method = "GET", body, responseMode = "json" } = {}) {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      catch (e) { if (operation >= latestSuccess) await markAuthorization(e, generation, operation); throw e; }
      assertCurrent(generation);
      let res;
      try {
      ...
      if (res.status === 401) {
        ...
        if (current && operation >= latestSuccess) await markAuthorization(error, generation, operation);
        throw error;
      }
      if (!res.ok) {
  ...
  FA.google = {
    ...
    async isConnected() {
      ...
      try {
        ...
        if (generation === connectionGeneration && operation >= latestSuccess) await markAuthorization(error, generation, operation);
        return false;
      }
    },
    ...
    async disconnect() {
      ...
      await chrome.storage.local.set({ [AUTH_STATE_KEY]: { ...emptyState(), userDisconnected: true, writer, session: null } });
      const generation = connectionGeneration;
      try {
