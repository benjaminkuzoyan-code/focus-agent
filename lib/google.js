...
const WEB_CLIENT_ID = "248896287493-4640r61qu86tcfqabid7v8mcbdk56kjm.apps.googleusercontent.com";
  ...
  async function rememberConnection(credential, generation = connectionGeneration) {
  ...
  async function getWebToken(interactive, generation) {
  ...
  async function getToken(interactive, generation = connectionGeneration) {
    ...
    try {
      ...
      try { return await getWebToken(interactive, generation); }
        ...
        throw authError(webError.category || "unknown", "web", [{ door: "chrome", category: e.category || "unknown" }, { door: "web", category: webError.category || "unknown" }]);
      }
    }
  }
  ...
  FA.google = {
    ...
    connect() {
      if (!pendingConnect) {
        ...
        const attempt = getToken(allowInteractive, generation).then(credential => rememberConnection(credential, generation)).then(() => true).finally(() => { if (pendingConnect === attempt) pendingConnect = null; });
    ...
    async disconnect() {
      ...
      try {
        if (credential?.door === "chrome") await forgetToken(credential);
