...
(() => {
  ...
  class MockCoach {
    ...
    readScreen() {
      return { fromClaude: false, error: "The real brain is offline — start the bridge and the coach can look at your screen." };
    }
    ...
    practiceTest(assignment, { files = [], count = 10 } = {}) {
      ...
      return {
        ...
        fromClaude: false,
        error: qs.length ? "" : "No highlights or definition-style sentences to build a test from yet — highlight key lines in the reading first, or start the bridge.",
      };
    }
  ...
  class ClaudeCoach extends MockCoach {
    ...
    async chat(messages, context = {}) {
      try {
        ...
        if (r.reply) return { reply: String(r.reply).slice(0, 40000) }; // essays are long; the panel scrolls
     ...
     */
    async readScreen(assignment, imageDataUrl, { question = "", page = {} } = {}) {
      try {
        ...
        return {
          ...
          keyIdeas: strs(r.keyIdeas, 6, 200),
          lookFor: strs(r.lookFor, 4, 200),
          questions: strs(r.questions, 4, 200),
          quotes: strs(r.quotes, 4, 200),
   ...
   */
  FA.resolveOpens = function resolveOpens(plan, resources = {}, { aggressive = false, maxOpens = 6 } = {}) {
    const links = resources.links || [];
    const topics = resources.topics || [];
