...
Generated from the code at v0.8.16 (2026-09-13), updated for v0.8.17 (2026-09-14: missing/overdue fix, attached links, photo) v0.8.18 (2026-09-15: PDF pages by eye, video summaries) v0.8.19 (2026-09-15: silent coach, whole school year) v0.8.20 (2026-09-15: overdue grace window, clock race fix) and v0.8.21 (2026-09-16: reply length cap, fold, scroll-to-top); **reviewed by Ben by voice 2026-09-13** — his direction is folded in below and marked 🎯 (decided) or 💬 (to discuss). Each section says what the student does, what happens, what must never happen, and what proves it. Status marks: ✅ proven by an automated test · 👁 checked by hand in the browser harness · ⚠️ implemented but never run for real · ❌ not built.
...
🎯 **Replies are short by default (decided 2026-09-16, ✅ v0.8.21).** Every chat and passage answer carries a LENGTH CAP: under ~120 words / 6 short lines, lead with the answer, at most one list of 4 bullets, no blank-line padding, no restating the question, no sign-off; longer only when the student explicitly asks for the whole thing ("walk me through", "explain fully", "step by step"). This is also the token budget: a reply's size is what a student's daily cap pays for. Developer mode keeps written work (essays, outlines, code) complete but drops the padding. Client backstop: 6 000 chars in tutor/answer mode, 40 000 in developer mode. In the panel a new coach reply opens with its **first line at the top** of the box (the student's own sends keep the box at the bottom), blank-line padding is collapsed, and a reply longer than 12 lines folds with "show more ▾". ✅ `security_tests.py` (cap present for students and for a developer with the switch off, absent with it on; passage answers capped) and the smoke test (fold / expand / padding).
